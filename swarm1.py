...
class Swarm1:
    ...
    def sequential(self, func, args_dict=None):
        ...
        """
        # 함수를 순차적으로 실행.
        # 함수의 첫 번째 인수는 작동할 Crazyflie에 연결된 SyncCrazyflie 인스턴스. 
        # 선택적 매개변수 목록(Crazyflie당)은 args_dict에 의해 정의됨. 
        # dictionary는 URI를 기준.
        for uri, cf in self._cfs.items():
    ...
    def parallel(self, func, args_dict=None):
        ...
        """
        # 함수를 병렬로 실행.
        # Crazyflie 당 하나의 스레드가 기능을 실행하기 시작.
        # 스레드는 마지막에 결합. 스레드에서 발생한 예외는 무시.
        try:
