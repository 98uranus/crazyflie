...
# Gains for velocity controller
Kp_vel = [.95, .95, 15.] # proportional [x,y,z]
Kd_vel = [1.8, 1.8, 15.]  # derivative [x,y,z]
Ki_vel = [0.2, 0.2, 1.0] # integral [x,y,z]
Ki_sat_vel = 0.1*np.ones(3)  # saturation for integral controller (prevent windup) [x,y,z]

# Gains for angle controller
Kp_ang = [10, 10, 25.] # proportional [x,y,z]
...
# Gains for angle_velocity controller
Kp_ang_vel = [10, 10, 25.] # proportional [x,y,z]
Kd_ang_vel = [3.7, 3.7, 9.]  # derivative [x,y,z]
Ki_ang_vel = [0.1, 0.1, 0.1] # integral [x,y,z]
Ki_sat_ang_vel = 0.1*np.ones(3)  # saturation for integral controller (prevent windup) [x,y,z]
...
vel_controller = PID_Controller(Kp_vel, Kd_vel, Ki_vel, Ki_sat_vel, dt)
angle_controller = PID_Controller(Kp_ang, Kd_ang, Ki_ang, Ki_sat_ang, dt)
angle_vel_controller = PID_Controller(Kp_ang_vel, Kd_ang_vel, Ki_ang_vel, Ki_sat_ang_vel, dt)
...
for time in enumerate(time_index):
    ...
    des_vel = pos_controller.control_update(pos_error)
    vel_error = quadcopter.calc_vel_error(quadcopter.vel)
    des_acc = pos_controller.control_update(vel_error) 
    ...
    # tau_needed = angle_controller.control_update(ang_vel_error)

    ang_error = quadcopter.calc_ang_error(quadcopter.angle)
    des_ang_vel = pos_controller.control_update(ang_error)
    ang_vel_error = quadcopter.calc_ang_vel_error(quadcopter.ang_vel)
    des_ang_acc = pos_controller.control_update(ang_vel_error)

    #Find motor speeds needed to achieve desired linear and angular accelerations
    quadcopter.des2speeds(thrust_needed, des_ang_acc)
