...
pos_des = np.array([[0., 0., 0.],  # desired position
                    [1., 1., 3.],
                    [1., 2., 3.],
                    [2., 2., 3.],
                    [2., 1., 3.],
...
# import sys
# sys.path.append("c:\\Users\\kangmin\\Desktop\\PythonWorkspace")
...
pos = np.array([pos_des[0,0], pos_des[0,1], pos_des[0,2]]) # starting location [x, y, z] ######받아와야함
...
time_indices = np.zeros(int((t[-1]-t[0])/dt))  # make empty time axis

### 계속 변하는 값

for i in range(len(pos_des)):

    time_index = np.arange(t[i], t[i+1], dt)
    r_ref = np.array([pos_des[i,0], pos_des[i,1], pos_des[i,2]]) #desired position [x, y, z]
    ...
    # make time axis
    time_indices[len(time_index)*i:len(time_index)*(i+1)] = time_index
...
def total_plot():
    ...
    axes.plot(time_indices, position[0], label= 'x')
    axes.plot(time_indices, position[1], label= 'y')
    ...
    axes.plot(time_indices, position[2], label= 'z')
    ...
    axes.plot(time_indices, velocity[0], label= 'd(x)/dt')
    axes.plot(time_indices, velocity[1], label= 'd(y)/dt')
    axes.plot(time_indices, velocity[2], label= 'd(z)/dt')
    ...
    axes.plot(time_indices, motor_thrust[0], label= 'motor 1')
    axes.plot(time_indices, motor_thrust[1], label= 'motor 2')
    axes.plot(time_indices, motor_thrust[2], label= 'motor 3')
    axes.plot(time_indices, motor_thrust[3], label= 'motor 4')
    ...
    axes.plot(time_indices, body_torque[0], label= 'x')
    axes.plot(time_indices, body_torque[1], label= 'y')
    axes.plot(time_indices, body_torque[2], label= 'z')
    ...
    axes.plot(time_indices, angle[0], label= 'phi')
    axes.plot(time_indices, angle[1], label= 'theta')
    axes.plot(time_indices, angle[2], label= 'psi')
    ...
    axes.plot(time_indices, angle_vel[0], label= 'd(phi)/dt')
    axes.plot(time_indices, angle_vel[1], label= 'd(theta)/dt')
    axes.plot(time_indices, angle_vel[2], label= 'd(psi)/dt')
