...
def calc_grid_map_config(xy_resolution):
    min_x = round(-6 - Sensor_Maxrange )
    min_y = round(-6 - Sensor_Maxrange )
    max_x = round(6 + Sensor_Maxrange )
    max_y = round(6 + Sensor_Maxrange )
...
def generate_ray_casting_grid_map(params, xy_resolution, occupancy_map):
    ...
    # laser_beams1 = np.empty((1, 2))
    # laser_beams1 = laser_beams1.astype('int32')
    # for i in range(4):
    #     laser = bresenham((icx1, icy1), (ix1[0, i], iy1[0, i]))
    #     laser_beams1 = np.vstack((laser_beams1, laser))
    
    # laser_beams2 = np.empty((1, 2))
    # laser_beams2 = laser_beams2.astype('int32')
    # for i in range(4):
    #     laser = bresenham((icx2, icy2), (ix1[0, i], iy1[0, i]))
    #     laser_beams2 = np.vstack((laser_beams2, laser))

    laser_beams3 = np.empty((1, 2))
    laser_beams3 = laser_beams3.astype('int32')
    for i in range(4):
        laser = bresenham((icx3, icy3), (ix4[0, i], iy4[0, i]))
    ...
    # for i in range(1,len(laser_beams1)):
    #     if occupancy_map[laser_beams1[i,0], laser_beams1[i,1]] != -0.2: # 레이저 빔 사이에 자취가 없으면
    #         occupancy_map[laser_beams1[i,0], laser_beams1[i,1]] = 0.0
    ...
    #     if occupancy_map[laser_beams3[i,0], laser_beams3[i,1]] != -0.2: # 레이저 빔 사이에 자취가 없으면
    #         occupancy_map[laser_beams3[i,0], laser_beams3[i,1]] = 0.0

    # for i in range(1,len(laser_beams4)):
    #     if occupancy_map[laser_beams4[i,0], laser_beams4[i,1]] != -0.2: # 레이저 빔 사이에 자취가 없으면
    ...
    # if params["radio://0/80/2M/E7E7E7E703"][0]['trace'][0]:
    #     occupancy_map[icx1][icy1] = -0.2
    #     params["radio://0/80/2M/E7E7E7E703"][0]['trace'][0] = False

    #     # trace = False
    #     print("TTTTTTTTTTTTTTTTTT11111111111111111111111111111111111111")
    # else:
    #     occupancy_map[icx1][icy1] = -0.4   # 로봇의 Location plotting

    # if params["radio://0/90/2M/E7E7E7E704"][0]['trace'][0]:
    #     occupancy_map[icx1][icy1] = -0.2
    #     params["radio://0/90/2M/E7E7E7E704"][0]['trace'][0] = False

    #     # trace = False
    #     print("TTTTTTTTTTTTTTTTTT22222222222222222222222222222222222222")
    # else:
    #     occupancy_map[icx1][icy1] = -0.4   # 로봇의 Location plotting
    ...
    # min_x, min_y, max_x, max_y, x_w, y_w = calc_grid_map_config(xy_resolution)
    # ox, oy = ox_and_oy(poses, measures)  # 좌표 계산 함수
    # icx, icy, ix, iy = \
    #     cell_number(poses, ox, oy, min_x, min_y, xy_resolution)  # 셀넘버 계산 함수
...
def turn(degree, mc, scf):
    rate = - 360 / 5
    turning = True
    
    mc.stop()
    if degree > 0:
        scf.cf.commander.send_hover_setpoint(0.3, 0.6, rate, 0.4)
            ...
            with Multiranger(scf) as multiranger:
                with Battery(scf) as Bat:
                    ...
                    while keep_flying:
                        ...
                        if uri == "radio://0/80/2M/E7E7E7E703": # main은 motion commander
                        ...
                        #         rate = 0
                            ...
                            if left > 0.7: # 왼쪽 벽이 멀면
                                rate, turning, trace = turn(90, mc, scf)
                                velocity_x = 0

                            else: # 왼쪽 벽이 가까우면
                                if front < 0.3: # 앞에 장애물이 있으면
                                    # print(front)
                                    rate, turning, trace = turn(-90, mc, scf)
                                    velocity_x = 0
                                else: # 앞에 장애물이 없으면
                        ...
                        if uri == "radio://0/80/2M/E7E7E7E703":
                            ...
                            print("AAAAAAAAAAAAaaaaaaaaaaasddddddddddddddddddA")
                        ...
                        #         poses2 = np.array([Xpos2, Ypos2, Yaw2])
                        #         hpose2 = np.vstack((hpose2, poses2))

                        #         poses4 = np.array([Xpos4, Ypos4, Yaw4])
                        #         hpose4 = np.vstack((hpose4, poses4))
                            
                        #     if front != None and left != None\
                        #         and back != None and right != None:
                        #         measures2 = np.array([front2, left2, back2, right2])

                        #         measures4 = np.array([front4, left4, back4, right4])
                        ...
                        # if uri == "radio://0/80/2M/E7E7E7E703":
                            ...
                            poses4 = np.array([
                                                -(params[URI4][0]['position'][0]),
                                                -(params[URI4][0]['position'][1])-1.5,
                                                params[URI4][0]['position'][2]+180
                                                ])
                            ...
                            ax2.plot(hposes3[:, 0],  # plot ground truth
                                    hposes3[:, 1], "-r", label='position2')
                            ax2.plot(hposes4[:, 0],  # plot ground truth
                                    hposes4[:, 1], "-b", label='position4')
                            
                            for i in range(4):
                                ax2.plot(hox3[:, i],    # plot pose(with noisy input)
                                        hoy3[:, i], ".y", label='measurements2')
                            for i in range(4):
                    ...
                    if uri == "radio://0/80/2M/E7E7E7E703":
                        ...
                        ani.save('good4.gif', fps=10000, dpi=80)   # .gif 로 저장
                        plt.show()
...
if __name__ == '__main__':
    ...
    # params1 = {'front': 0.1, 'trace': True}
    # params2 = {'front': 0.2, 'trace': True}
    params3 = {'front': 0.3, 'trace': True}
    params4 = {'front': 0.4, 'trace': True}
