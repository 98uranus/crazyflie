...
sequence = [
    (0, 0, 0.4, 0),
    (0.5/np.sqrt(2), 0, 0.4, 0),
    (0, 0.5/np.sqrt(2), 0.4, 0),
    (-0.5/np.sqrt(2), 0, 0.4, 0),
    (0, -0.5/np.sqrt(2), 0.4, 0),
    (0.5/np.sqrt(2), 0, 0.4, 0),
    (0.5/np.sqrt(2), 0, 0.4, 0),
    (0.5/np.sqrt(2), 0, 0.2, 0)
]
...
class KalmanV:  # drone의 현재 위치 Logging
    A = 'kalman.statePX'
    B = 'kalman.statePY'
    C = 'kalman.statePZ'
    ...
    def _data_received(self, timestamp, data, logconf):
        self._state_vx = data[self.A]
        self._state_vy = data[self.B]
        self._state_vz = data[self.C]

    @property
    def state_vx(self):
        return self._state_vx
    @property
    def state_vy(self):
        return self._state_vy
    @property
    def state_vz(self):
        return self._state_vz
    def __enter__(self):
        self.start()
        return self
    def __exit__(self, exc_type, exc_val, exc_tb):
...
def acc_gyro_log(state, acc_x, acc_y, acc_z, gyro_x, gyro_y, gyro_z, pos_x, pos_y, pos_z) : 
    
    with open('acc_gyro_pos_log.csv', 'a', newline='') as f : 
        # Accelerometer data
        a = acc_x
        b = acc_y
        c = acc_z
        ...
        z = pos_z       
        wr = csv.writer(f)
        wr.writerow([state,a,b,c,d,e,g,x,y,z])
                    ...
                    # with Gyro(scf) as gyro:
                        # with KalmanV(scf) as ekfv:
                            ...
                            while keep_flying:
                                ...
                                # acc_gyro_log(state,a,b,c,d,e,f,x,y,z)
