...
def updatefig(*args):
    global xy_resolution, occupancy_map, keep_flying
    global scf, multiranger, Ste, motion_commander

    if keep_flying:

        velocity_x = is_front(multiranger.front)
        velocity_y = is_left(multiranger.left)
        # front_left(multiranger.front, multiranger.left)

        if multiranger.up < 0.1:
            keep_flying = False
...
#     if(range_f < 0.4 and range_l < 0.4):
#         motion_commander.turn_right(90)
        ...
        with State(scf) as Ste:
            with MotionCommander(scf) as motion_commander: # 이 라인을 활성화할 경우 갑자기 이륙할 수 있으므로 주의
                with Multiranger(scf) as multiranger:
                    ...
                    # ani.save('Kangmin_house', writer='imagemagick')   # .gif 로 저장
                    plt.show()